...
echo "       Pixelcade for Recalbox : Installer Version $version    "
echo ""
echo "This script will install Pixelcade in your /recalbox/share folder"
...
# let's make sure we have Recalbox installation
if [[ -d "/recalbox/share" ]]; then #if the user killed the installer mid-stream,it's possible this file is still there so let's remove it to be sure before downloading, otherwise wget will download and rename to .1
     echo "Recalbox Detected"
else
   echo "Sorry, Recalbox was not detected, exiting..."
   exit 1
fi
...
updateartwork() {  #this is needed for rom names with spaces
  ...
  curl -LO https://github.com/alinke/pixelcade/archive/refs/heads/master.zip
  unzip master.zip
